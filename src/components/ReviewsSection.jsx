        ...
        {reviews.length > 0 ? (
          <div className="grid gap-6 md:grid-cols-2">
            {reviews.map((review, index) => (
              <blockquote key={index} className="bg-white p-8 rounded-md shadow-sm border-l-4 border-orange">
                <p className="text-neutral-800 italic leading-relaxed md:text-lg">“{review.quote}”</p>
                <footer className="mt-4 font-semibold text-darkblue md:text-lg">– {review.name}</footer>
              </blockquote>
            ))}
          </div>
        ) : (
