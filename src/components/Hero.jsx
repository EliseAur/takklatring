...
export default function Hero({ hero }) {
  ...
  return (
    <section
      id="top"
      className="overflow-hidden w-full min-h-[calc(100vh-89px)] lg:min-h-[calc(100vh-104px)]"
    >
      <div className="heroImage relative flex min-h-[calc(100vh-89px)] lg:min-h-[calc(100vh-104px)]">
        <img
          src={imageSrc}
          alt={imageAlt}
          className="absolute inset-0 h-full w-full object-cover object-center md:object-right bg-darkblue origin-right lg:pl-96"
        />
        ...
        <div className="relative z-20 my-auto flex flex-col md:mx-20 px-7 md:px-0 h-full max-w-[500px] md:max-w-[540px]">
          ...
          <p className="text-neutral-100 md:hidden font-body text-sm md:text-lg mb-6 font-bold drop-shadow-[0_1px_2px_rgba(0,0,0,0.9)]">
            {descriptionMobile}
          </p>
            ...
            {usps.map((text, i) => (
              <div key={i} className="flex flex-row gap-2 items-center">
                <img
                  src={iconVerified}
                  alt=""
                  className="w-7 h-7 md:w-8 md:h-8 drop-shadow-[0_1px_2px_rgba(0,0,0,0.9)]"
                />
